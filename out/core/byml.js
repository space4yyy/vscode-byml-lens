...
exports.yamlToByml = yamlToByml;
exports.bymlToYaml = bymlToYaml;
...
class Writer {
    ...
    writeUInt8(v) { this.view.setUint8(this.offset++, v); }
    writeUInt16(v) { this.view.setUint16(this.offset, v, this.le); this.offset += 2; }
    writeUInt32(v) { this.view.setUint32(this.offset, v, this.le); this.offset += 4; }
    writeUInt24(v) {
        if (this.le) {
...
function yamlToByml(yamlStr, originalData) {
    ...
    if (originalData) {
        ...
        if (decompressed[0] === 0x42 && decompressed[1] === 0x59)
            le = false;
    ...
    function writeNode(node) {
        ...
        if (Array.isArray(node)) {
            ...
            for (const item of node)
                writer.writeUInt32(0); // Placeholders
...
function bymlToYaml(data) {
    ...
    if (magic === 'BY')
        ...
        throw new Error('Invalid BYML magic: ' + magic);
    ...
    function parseNode(offset) {
        ...
        if (type === 0xC0) { // Array
            ...
            for (let i = 0; i < count; i++)
                types.push(reader.readUInt8());
            while (reader.tell() % 4 !== 0)
                reader.readUInt8();
            const arr = [];
            for (let i = 0; i < count; i++) {
    ...
    function parseValue(type, value) {
        switch (type) {
            ...
            case 0xC0:
            case 0xC1: return parseNode(value);
...
class Reader {
    ...
    readStringTable(offset) {
        ...
        if (type !== 0xC2)
            throw new Error('Invalid string table type at 0x' + offset.toString(16) + ': 0x' + type.toString(16));
